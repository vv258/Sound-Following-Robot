...
time13=time1-time3;
    ...
    if(tdoa1_error>tdoa1_threshold){
            ...
            if((tdoa1_error<0 && tdoa1_last_error>0) ||(tdoa1_error>0 &&t tdoa1_last_error<0))
            ...
            if((tdoa2_error<0 && tdoa2_last_error>0) ||(tdoa2_error>0 &&t tdoa2_last_error<0))
...
void __ISR(_INPUT_CAPTURE_2_VECTOR, ipl4) C1Handler(void)
{
    // read the capture register 
...
void __ISR(_INPUT_CAPTURE_5_VECTOR, ipl5) C1Handler(void)
{
    // read the capture register 
...
timerlimit = SYS_FREQ* PWM_Period/1000;
pwm_on_time_idle=1.5*SYS_FREQ/1000;  
pwm_on_time_max=1.7*SYS_FREQ/1000;
pwm_on_time_min=1.3*SYS_FREQ/1000;
tdoa1_threshold=SYS_FREQ/100000;

OpenTimer2(T2_ON | T2_SOURCE_INT | T2_PS_1_1, timerlimit);
